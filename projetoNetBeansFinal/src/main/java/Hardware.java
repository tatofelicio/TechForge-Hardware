...
public abstract class  Hardware implements Promocional{
    ...
    public void setPreco(double preco) throws PrecoInvalidoException {
        if (preco <= 0){
            throw new PrecoInvalidoException("Operação cancelada: O preço do componente não pode ser negativo ou zero!"); //valida precos negativos
        }
        this.preco = preco;
    }
    ...
    public void setQtdEstoque(int qtdEstoque) throws EstoqueInvalidoException, EstoqueExcedidoException {
        if (qtdEstoque < 0) {
            throw new EstoqueInvalidoException("A quantidade em estoque não pode ser menor que zero!"); //valida números negativos
        } else if (qtdEstoque > 10) {
            throw new EstoqueExcedidoException("O estoque físico não comporta essa quantidade de peças!"); //valida excesso de estoque
        }
